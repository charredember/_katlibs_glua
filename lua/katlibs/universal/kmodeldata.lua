...
function KModelData.FromSerializable(serializable)
    ...
    local color = serializable.Color
    if color then
...
---CLIENT<br/>
---Gets the KRenderProperties of this KModelData.
---@return KRenderProperty[]
function KModelData:GetRenderProperties()
    local priv = getPriv(self)
    local renderProperties = {}

    local color = priv.Color
    local defaultColor = defaultValues.Color
    if color and (color.r ~= defaultColor.r or color.g ~= defaultColor.g or color.b ~= defaultColor.b) then
        table.insert(renderProperties,KRenderProperty.ColorModulation(color.r / 255, color.g / 255, color.b / 255))
    end

    local material = priv.Material
    if material and material ~= defaultValues.Material then
        table.insert(renderProperties,KRenderProperty.ModelMaterialOverride(material))
    end
...
---CLIENT<br/>
---Gets the KStudioProperties of this KModelData.
---@return KStudioProperty
function KModelData:GetStudioProperties()
    local priv = getPriv(self)
    return KStudioProperty(priv.RenderMode, priv.RenderGroup)
end